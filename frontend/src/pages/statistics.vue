    ...
    <div class="container">
      <div class="statisticks">
        <div class="statisticks__header row mb-4 align-items-center">
            <div class="statisticks__header-title statisticks__main-total-title col-5">
        ...
        <div class="statisticks__filter d-flex justify-content-start">
            ...
            <div v-if="subCategoriesVisible && subCategoriesOptions" class="statisticks_dropdown-wrap">
              <DropdownBlock
                :options="[
                  {
                    value: 0,
...
import {get, mapState} from 'vuex-pathify';
...
export default {
  ...
  methods: {
    async changeCategory(value) {
      ...
      this.$store.commit('statistics/filterObjectsStat', {'field': 'main_category_id', 'value': value});
      this.$store.commit('statistics/filterObjectsStat', {'field': 'category_id', 'value': 0});
      await this.$store.dispatch('statistics/getObjectsStatTable');
    },
    changeSubcategory(value) {
      this.$store.commit('statistics/filterObjectsStat', {'field': 'category_id', 'value': value});
      this.selectedSubCategory = value
      this.$store.dispatch('statistics/getObjectsStatTable');
    },
    changeCity(value) {
      this.$store.commit('statistics/filterObjectsStat', {'field': 'city_id', 'value': value});
      this.selectedCity = value
      this.$store.dispatch('statistics/getObjectsStatTable');
    },
    async getSubCategories(value) {
      const categories = [...this.categories]
      this.subCategoriesOptions = await categories.find(cat => cat.id == value);
      this.subCategoriesVisible = true;
    },
    async exportList() {
      window.open(`/api/objects/statistic/export/excel?city_id=${this.selectedCity ? this.selectedCity : 0}&main_category_id=${this.selectedCategory ? this.selectedCategory : 0}&category_id=${this.selectedSubCategory ? this.selectedSubCategory : 0}&group=${this.selectedGroup}`, '_blank')
    }
  },
};
</script>
