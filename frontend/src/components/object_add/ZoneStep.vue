<template>
  <div class="add-object__content">
    <attributes-list
      ...
      @change="
        updateData({
          path: `${zoneKey}.attributes.${$event.path}`,
          value: $event.value
        })
      "
    />
    <accessibility-score
      ...
      :key="zoneType"
    />
    <div class="add-object__line --lrg add-object__line-comment">
      <h5 class="add-object__title">
        {{ $t("objectAdding.zoneCommentLabel") }}
      </h5>
    </div>
    <div class="add-object__line add-object__textarea">
      <textarea
        class="textarea"
        v-model="comment"
        :placeholder="$t('objectAdding.zoneCommentPlaceholder')"
      >
      </textarea>
    </div>
  </div>
