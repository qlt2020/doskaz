<template>
    <div>
        <attributes-list :form="form" zone="kidsAccessibility" :value="value.attributes" @input="update"/>
        <textarea-field label="Комментарий" :path="`${path}.comment`" :disabled="true"/>
        <h4>Оценка доступности</h4>
        <hr/>
        <accessibility-score :allow-override="true" :value="value" @input="overrideScore" :type="`kidsAccessibility_${form}`"/>
    </div>
</template>
