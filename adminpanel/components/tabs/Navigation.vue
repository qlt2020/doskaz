<template>
    <div>
        <attributes-list :form="form" zone="navigation" :value="value.attributes" @input="update"/>

        <textarea-field label="Комментарий" :path="`${path}.comment`" :disabled="true"/>

        <h4>Оценка доступности</h4>
        <hr/>
        <accessibility-score :allow-override="true" :value="value"  @input="overrideScore" :type="`navigation_${form}`"/>
    </div>
</template>
